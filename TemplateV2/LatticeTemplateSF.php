...
class LatticeTemplateSF extends LatticeTemplateBase
{
    /**
     * 状态1模板
    ...
    )
    {
        ...
        $spacing = 0;
        // 同城标快
        $lattice->text($orderInfo['sf_type'], $fontSize, [$x, $fontY], 1, $spacing, $bold);
        $lattice->text('已检视', 12, [$x, $fontY + 17], 1, $spacing, 0);

        // 三段码
        $lattice->text($orderInfo['numbering'], 16, [0, 3, 'top-center'], 1, 0, 1, 1);
        // 一维码
        LatticeImg::BarCode($lattice, $orderInfo['courier_number'], 42, [-10, 30, 'top-right'], 1, $this->imgPath, $this->imgName, 1);
        $x = 1;
        // 中转包提示
        if (!empty($orderInfo['transfer']))
        {
            $lattice->text($orderInfo['transfer'], 12, [$x, -10, 'center-left'], 1, 0, 1, 1);
        }
        // 打印时间
        $date = Date('Y-m-d H:i');
        $lattice->text($date, 12, [$x, 3, 'center-left'], 1, 0, 0);
        ...
        // 手机号 脱敏
        $phone = mb_substr($orderInfo['consignee_phone'], 0, 3) . '****' . mb_substr($orderInfo['consignee_phone'], -4, 4);
        $lattice->text($phone, $fontSize, [$x + $cd * 8, $fontY], 0, $spacing, $bold);

        // 物流ID
        $lattice->text('顺丰 ' . $orderInfo['courier_number'], 12, [$x - 10, 17, 'center-right'], 0, 0, 0, 1);
        ...
        $strlen = 0; // 到付的长度
        // 有到付的情况下绘制到付
        if (!empty($orderInfo['payde'])) {

            $fontSize = 12;
            $text = '到付:' . $orderInfo['payde'];
            $strlen = (mb_strlen($orderInfo['payde'], 'UTF-8') * 7) + ($fontSize * 3);
            // 绘制白边
            LatticeImg::Rectangle($lattice, 2, 16, [-$strlen, -2, 'bottom-right'], 0);
            // 绘制黑边
            LatticeImg::Rectangle($lattice, $strlen, 16, [0, -3, 'bottom-right'], 1);
            // 写到付
            $lattice->text($text, $fontSize, [-1, -1, 'bottom-right'], 0, 0, 0);

            // 绘制白边
            LatticeImg::Rectangle($lattice, $strlen, 2, [0, -5, 'bottom-right'], 0);

        }elseif (!empty($orderInfo['payde_jf'])){
            $fontSize = 12;
            $text = '寄付现结:' . $orderInfo['payde_jf'];
            $strlen = (mb_strlen($orderInfo['payde_jf'], 'UTF-8') * 7) + ($fontSize * 3) + 20;
            // 绘制白边
            LatticeImg::Rectangle($lattice, 2, 16, [-$strlen, -2, 'bottom-right'], 0);
            // 绘制黑边
            LatticeImg::Rectangle($lattice, $strlen, 16, [0, -3, 'bottom-right'], 1);
    ...
    )
    {
        ...
        $y = 1;
        // 物流公司
        $express = '百世快递' . $data['order_info']['courier_number'];
